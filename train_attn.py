...
from util.visualizer import save_images_v3
...
if __name__ == '__main__':
    ...
    for epoch in range(opt.epoch_count, opt.niter + opt.niter_decay + 1):    # outer loop for different epochs; we save the model by <epoch_count>, <epoch_count>+<save_latest_freq>
        ...
        if epoch == 20 or epoch == 40 or epoch == 60:
            web_dir = os.path.join('./results/', opt.name, '%s_%s' % ('attn', str(epoch)))
            webpage = html.HTML(web_dir, 'Experiment = %s, Phase = %s, Epoch = %s' % (opt.name, 'attn', str(epoch)))
