...
#             if subdirectory.endswith("SmoothDisc") or subdirectory.endswith("Side"):
#                 continue
#             else:
#                 shutil.rmtree(subdirectory) # Remove all the subdirectories that don't contain Disc shaped or side view RBCs
...
#     subdirectories = [x[0] for x in os.walk(folder_path)]
#     for subdirectory in subdirectories:
#         if subdirectory.endswith("Side") or subdirectory.endswith("SmoothDisc"): # Enter the folder where images are stored
#             img_paths = []
...
#                 try:
#                     if not img_path.endswith("Ch1.ome.tif"):
...
for folder in avail_folders:
    ...
    subdirectories = [x[0] for x in os.walk(folder_path)]
    for subdirectory in subdirectories:
        if subdirectory.endswith("SmoothDisc") or subdirectory.endswith("Side"): # Enter the folder where images are stored
            img_paths = []
            ...
            for x in os.walk(subdirectory): # Get list of image files present in the directory
                img_paths.extend(x[2])
            for img_path in img_paths:
                # Randomly select 50 % of all images since the total dataset length was otherwise 6102 images
